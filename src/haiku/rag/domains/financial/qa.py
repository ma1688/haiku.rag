...
from .prompts import (FINANCIAL_SYSTEM_PROMPT, get_intent_prompt, ANSWER_FORMATS, ERROR_MESSAGES)
...
class FinancialQuestionAnswerAgent(QuestionAnswerAgentBase):
    ...
    def _create_enhanced_tools(self) -> list:
        ...
        return [{"type": "function",
            "function": {"name": "search_financial_documents", "description": "搜索金融公告文档，支持多种过滤条件",
                "parameters": {"type": "object", "properties": {
                    "query": {"type": "string", "description": "搜索查询词（支持公司名、股票代码、交易类型等）", },
                    "limit": {"type": "integer", "description": "返回结果数量限制", "default": 5, },
                    "announcement_type": {"type": "string", "description": "公告类型过滤",
                        "enum": ["earnings", "acquisition", "connected_transaction", "all"], "default": "all"},
                    "date_range": {"type": "object", "description": "日期范围过滤",
                        "properties": {"start_date": {"type": "string", "format": "date"},
                            "end_date": {"type": "string", "format": "date"}}}}, "required": ["query"], }, }, },
            {"type": "function",
                "function": {"name": "extract_financial_data", "description": "从文档中提取结构化的财务数据",
                    "parameters": {"type": "object",
                        "properties": {"document_content": {"type": "string", "description": "要分析的文档内容", },
                            "data_types": {"type": "array", "description": "需要提取的数据类型",
                                "items": {"type": "string",
    ...
    async def _enhance_search_results(self, results: List[Tuple[Chunk, float]], query: str) -> List[Dict]:
        """增强搜索结果，添加结构化信息"""
        enhanced_results = []
        ...
        for chunk, score in results:
            ...
            enhanced_results.append(
                {"content": chunk.content, "score": score, "chunk_id": chunk.id, "document_id": chunk.document_id,
                    "announcement_type": announcement_type, "metadata": metadata})
    ...
    async def answer(self, question: str) -> str:
        ...
        try:
            ...
            for result in enhanced_results:
                extracted = await self._extract_key_information(result["content"], question.lower())
                if extracted:
            ...
            # 7. 生成结构化回答
            response_data = {"query": question, "intent": intent_prompt[:50] if intent_prompt else "general",
                "results_count": len(enhanced_results), "extracted_info": all_extracted_info, "context": context,
            ...
            if all_extracted_info:
                ...
                formatted_response = await self._format_financial_data(
                    {"financial_data": json.dumps(all_extracted_info, ensure_ascii=False, indent=2),
                        "source_reference": response_data["source_reference"], "company_name": "相关公司",
                        # 可以从context中提取
                        "announcement_type": enhanced_results[0][
                            "announcement_type"] if enhanced_results else "general"})
...
class FinancialQuestionAnswerOpenAIAgent(FinancialQuestionAnswerAgent):
    ...
    def _init_openai_client(self):
        ...
        try:
            ...
            self.tools: Sequence[ChatCompletionToolParam] = [ChatCompletionToolParam(tool) for tool in
    ...
    async def answer(self, question: str) -> str:
        ...
        for _ in range(max_rounds):
            response = await self.openai_client.chat.completions.create(model=self._model, messages=messages,
                tools=self.tools, temperature=0.1,  # 低温度以获得更一致的金融数据
            )

            response_message = response.choices[0].message

            if response_message.tool_calls:
                messages.append(ChatCompletionAssistantMessageParam(role="assistant", content=response_message.content,
                ...
                for tool_call in response_message.tool_calls:
                    if tool_call.function.name == "search_financial_documents":
                        ...
                        # 构建工具响应
                        tool_response = {"results": enhanced_results, "total": len(enhanced_results)}

                        messages.append(ChatCompletionToolMessageParam(role="tool",
                            content=json.dumps(tool_response, ensure_ascii=False), tool_call_id=tool_call.id, ))
                        ...
                        # 提取财务数据
                        extracted_data = await self._extract_key_information(content, question.lower())

                        messages.append(ChatCompletionToolMessageParam(role="tool",
                            content=json.dumps(extracted_data, ensure_ascii=False), tool_call_id=tool_call.id, ))
            else:
                # 没有工具调用，返回最终答案
