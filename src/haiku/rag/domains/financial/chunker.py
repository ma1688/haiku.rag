...
class FinancialChunker(Chunker):
    ...
    SECTION_PATTERNS = [# Numbered sections (1. 2. 1.1 etc)
        r'^[\s]*(\d+\.[\d\.]*)\s+(.+)$', # Chinese numbered sections (一、二、(一) etc)
        r'^[\s]*([一二三四五六七八九十]+[、.])\s*(.+)$', r'^[\s]*[(（][一二三四五六七八九十]+[)）]\s*(.+)$',
        # Letter sections (A. B. (a) etc)
        r'^[\s]*([A-Z]\.)\s+(.+)$', r'^[\s]*[(（][a-zA-Z][)）]\s*(.+)$', # Special keywords that often start sections
    ...
    # Table indicators
    TABLE_INDICATORS = [r'[-─━]+\s*[-─━]+',  # Table borders
    ...
    def __init__(self, chunk_size: int = 1500,  # Larger for financial docs
            chunk_overlap: int = 400,  # More overlap for context
            preserve_tables: bool = True, extract_metadata: bool = True):
