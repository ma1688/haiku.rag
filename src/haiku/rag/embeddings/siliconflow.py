    ...
    class Embedder(EmbedderBase):
        ...
        def __init__(self, model: str, vector_dim: int):
            ...
            if not self._api_key:
                raise ValueError(
                    "SILICONFLOW_API_KEY environment variable is required for SiliconFlow embeddings"
                )
            ...
            async with httpx.AsyncClient() as client:
                try:
                    ...
                    if len(embedding) != self._vector_dim:
                        raise ValueError(
                            f"Expected embedding dimension {self._vector_dim}, "
                    ...
                    raise RuntimeError(
                        f"SiliconFlow API error (status {e.response.status_code}): {error_detail}"
                    ) from e
